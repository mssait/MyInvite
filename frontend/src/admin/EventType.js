...
export const EventType = () => {
  ...
  const processRowUpdate = async (newRow, oldRow) => {
    // Fixed: Changed "music" to "event_type" to match backend
    const data = {
      event_type: newRow.name,
    };

    if (!data.event_type) {
      enqueueSnackbar("Event type should not be empty", { variant: "warning" });
      setRowModesModel({
        ...rowModesModel,
        [oldRow.id]: {
    ...
    if (typeof newRow.id === "number") {
      url = `/api/master/edit-event-types/${newRow.id}`;
      method = "put";
      successMessage = "Edited successfully";
    } else if (newRow.id.indexOf("new-") === 0) {
      url = `/api/master/add-event-types`;
      method = "post";
  ...
  return (
    <Stack spacing={2}>
      <Card>
        <CardContent>
          <Box display={{ md: "flex", xs: "column" }} justifyContent="space-between" alignItems="center">
            <Box>
              <Typography variant="h3" pb={{ md: 0, xs: 1 }}>Event Types</Typography>
            </Box>
      ...
      <ClientDataGrid
        ...
        ajax={{ url: "/api/master/event-types" }}
        columns={[
          {
            headerName: "Action",
            field: "Action",
              ...
              return isInEditMode
                ? [
                  <Chip
                    ...
                    label="Save"
                    variant="contained"
                    sx={{ bgcolor: green[100], color: green[900], borderRadius: "8px" }}
                    onClick={() => {
                      setRowModesModel({
                        ...rowModesModel,
                        [id]: {
